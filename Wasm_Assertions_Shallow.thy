...
definition valid_triple :: "'a triple_context \<Rightarrow> 'a ass \<Rightarrow> e list \<Rightarrow> 'a ass \<Rightarrow> bool" ("_ \<Turnstile> {_}_{_}" 60) where
  "(\<Gamma> \<Turnstile> {P}es{Q}) \<equiv> \<forall>vcs h st s locs labs labsf ret retf lvar_st hf vcsf s' locs' res.
                                      ...
                                      ((s,\<lparr> f_locs=locs, f_inst=i \<rparr>,($C*vcsf)@($C*vcs)@es) \<Down>{(labs@labsf,case_ret ret retf)} (s',\<lparr> f_locs=locs', f_inst=i \<rparr>, res))) \<longrightarrow>
...
definition valid_triple_n :: "'a triple_context \<Rightarrow> nat \<Rightarrow> 'a ass \<Rightarrow> e list \<Rightarrow> 'a ass \<Rightarrow> bool" ("_ \<Turnstile>'_ _ {_}_{_}" 60) where
  "(\<Gamma> \<Turnstile>_k {P}es{Q}) \<equiv> \<forall>vcs h st s locs labs labsf ret retf lvar_st hf vcsf s' locs' res.
                                      ((ass_wf lvar_st ret \<Gamma> labs locs s hf st h vcs P \<and>
                                      ((s,\<lparr> f_locs=locs, f_inst=i \<rparr>,($C*vcsf)@($C*vcs)@es) \<Down>k{(labs@labsf,case_ret ret retf)} (s',\<lparr> f_locs=locs', f_inst=i \<rparr>, res))) \<longrightarrow>
                                      res_wf lvar_st \<Gamma> res locs' s' hf vcsf Q)"
...
lemma res_wf_valid_triple_n_intro:
  assumes "\<Gamma> \<Turnstile>_k {P}es{Q}"
          ...
          "((s,\<lparr> f_locs=locs, f_inst=i \<rparr>,($C*vcsf)@($C*vcs)@es) \<Down>k{(labs@labsf,case_ret ret retf)} (s',\<lparr> f_locs=locs', f_inst=i \<rparr>, res))"
...
lemma res_wf_valid_triple_n_intro_tight:
  assumes "\<Gamma> \<Turnstile>_k {P}es{Q}"
          "ass_wf lvar_st ret \<Gamma> labs locs s hf st h vcs P"
          "((s,\<lparr> f_locs=locs, f_inst=i \<rparr>,($C*vcsf)@($C*vcs)@es) \<Down>k{(labs,ret)} (s',\<lparr> f_locs=locs', f_inst=i \<rparr>, res))"
  shows "res_wf lvar_st \<Gamma> res locs' s' hf vcsf Q"
...
proof -
  {
    ...
    assume local_assms:"ass_wf lvar_st ret (fs, ls', rs') labs locs s hf st h vcs P"
                       "(s, \<lparr> f_locs=locs, f_inst=i \<rparr>, ($C* vcsf) @ ($C* vcs) @ [$Call j]) \<Down>n{(labs@labsf, case_ret ret retf)} (s', \<lparr> f_locs=locs', f_inst=i \<rparr>, res)"
    obtain ret' labs' where "ass_wf lvar_st ret' (fs, ls, rs) labs' locs s hf st h vcs P"
      ...
      by fastforce
    moreover
    have "(s, \<lparr> f_locs=locs, f_inst=i \<rparr>, ($C* vcsf) @ ($C* vcs) @ [$Call j]) \<Down>n{(labs'@labsf, case_ret ret' retf)} (s', \<lparr> f_locs=locs', f_inst=i \<rparr>, res)"
...
proof -
  {
    ...
    proof (cases res)
      case (RValue x1)
      hence "(s, \<lparr> f_locs=locs, f_inst=i \<rparr>, ($C*vcsf)@($C*vcs)@es) \<Down>n{(labs@labsf', case_ret ret retf')} (s', \<lparr> f_locs=locs', f_inst=i \<rparr>, res)"
    ...
    next
      ...
      have 0:"ib < length (labs'@labsf')"
        ...
        by fastforce
      have b0:"(s, \<lparr> f_locs=locs, f_inst=i \<rparr>, ($C*vcsf)@($C*vcs)@es) \<Down>n{(labs'@labsf', case_ret ret' retf')} (s', \<lparr> f_locs=locs', f_inst=i \<rparr>, RBreak ib vbs)"
      ...
      proof (cases "labs!ib \<noteq> labs'!ib")
        ...
        hence 1:"\<And>vcs h st. ib < length labs' \<Longrightarrow> \<not> ass_sat (ls!ib) vcs h st"
          ...
          by fastforce+
        obtain vbs' where 2:"((s, \<lparr> f_locs=locs, f_inst=i \<rparr>, ($C*vcsf)@($C*vcs)@es) \<Down>n{(labs@labsf', case_ret ret retf')} (s', \<lparr> f_locs=locs', f_inst=i \<rparr>, RBreak ib vbs'))"
    ...
    next
      ...
      proof (cases "(pred_option (\<lambda>r_r. (case_ret ret retf') \<noteq> Some r_r) (case_ret ret' retf'))")
        ...
        then obtain vrs' where "((s, \<lparr> f_locs=locs, f_inst=i \<rparr>, ($C*vcsf)@($C*vcs)@es) \<Down>n{(labs@labsf', case_ret ret retf')} (s', \<lparr> f_locs=locs', f_inst=i \<rparr>, RReturn vrs'))"
