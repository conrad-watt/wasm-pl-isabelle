...
lemma reduce_to_imp_reduce_trans:
  ...
  shows "(res = RTrap \<longrightarrow> reduce_trans (s,vs,es) (s',vs',[Trap])) \<and>
         (\<forall>rvs. (res = RValue rvs \<longrightarrow> reduce_trans (s,vs,es) (s',vs',$$*rvs))) \<and>
         (\<forall>n rvs lholed lfes les. (res = RBreak n rvs \<longrightarrow> (Lfilled n lholed es lfes \<longrightarrow> (ls!n = length rvs \<and> reduce_trans (s,vs,[Label (ls!n) les lfes]) (s',vs',($$*rvs)@les))))) \<and>
         (\<forall>n rvs lholed lfes v0s. (res = RReturn rvs \<longrightarrow> (Lfilled n lholed es lfes \<longrightarrow> (r = Some (length rvs) \<and> reduce_trans (s,v0s,[Local (length rvs) vs lfes]) (s',v0s,$$*rvs)))))"
  using assms
...
next
  ...
  hence 1:"reduce_trans (s, vs, es) (s', vs', $$* res)"
    by simp
  show ?case
...
next
  ...
  hence 1:"reduce_trans (s, f, es)  (s', f',$$* res)"
    by fastforce
  have "\<lparr>s'; f'; [Label n les ($$* res)]\<rparr> \<leadsto> \<lparr>s'; f'; $$* res \<rparr>"
...
next
  ...
  hence 1:"reduce_trans (s, fl, es) (s', fl', $$* res)"
    by simp
  have "reduce_trans (s, f, [Local n fl es]) (s', f, [Local n fl' ($$* res)])"
    using reduce_trans_local[OF 1]
    by blast
  moreover
  have "\<lparr>s'; f; [Local n fl' ($$* res)]\<rparr> \<leadsto> \<lparr>s'; f; ($$* res)\<rparr>"
...
next
  ...
  hence 0:"reduce_trans (s, vs, es) (s'', vs'', $$* res'')"
    by simp
  have 1:"reduce_trans (s, vs, es@es') (s'', vs'', ($$* res'')@es')"
    using reduce_trans_L0[OF 0, of "[]"]
    by fastforce
  show ?case
  proof (cases res')
    case (RValue x1)
    hence "reduce_trans (s'', vs'', ($$* res'') @ es') (s', vs', $$* x1)"
      using seq_value
      by simp
    thus ?thesis
  ...
  next
    case RTrap
    hence "reduce_trans (s'', vs'', ($$* res'') @ es') (s', vs', [Trap])"
      using seq_value
      by simp
    thus ?thesis
...
next
  ...
  have 0:"reduce_trans (s, vs, [Label n les es]) (s'', vs'', ($$* bvs) @ les)"
    using label_break_nil(2) Lfilled_exact.L0 Lfilled_exact_imp_Lfilled
    by simp blast
  hence 1:"reduce_trans (s, vs, ($$* vcs) @ [Label n les es]) (s'', vs'', ($$*vcs @ bvs) @ les)"
    using reduce_trans_L0[OF 0, of vcs "[]"]
    by fastforce
  show ?case
  proof (cases res)
    case (RValue x1)
    hence "reduce_trans (s'', vs'', ($$* vcs @ bvs) @ les) (s', vs', $$* x1)"
  ...
  next
    case RTrap
    hence "reduce_trans (s'', vs'', ($$* vcs @ bvs) @ les) (s', vs', [Trap])"
      using label_break_nil
      by simp
    thus ?thesis
      using RTrap reduce_trans_compose 1
...
proof (induction j lholed "(($$*vcs) @ [$Return])" es arbitrary: ls rule: Lfilled.induct)
  ...
  have "(s, vs, ($$* vcs) @ [$Return]) \<Down>{(ls, Some n)} (s, vs, RReturn vcs)"
...
proof (induction j lholed "(($$*vcs) @ [$Br (k+j)])" es arbitrary: ls k rule: Lfilled.induct)
  case (L0 vls lholed es')
  hence "(s, vs, ($$* vcs) @ [$Br (k)]) \<Down>{(ls, r)} (s, vs, RBreak k vcs)"
    using reduce_to.br
    by fastforce
  thus ?case
    using reduce_to_L0 L0
...
lemma reduce_to_n_lfilled_context:
  ...
  shows "\<exists>ls' vcs s'' vs'' res' lholed. ((s, vs, (($$*vcs)@es)) \<Down>{(ls',r)} (s'', vs'', res')) \<and>
                                     Lfilled k lholed (($$*vcs)@es) les"
  using assms
proof (induction arbitrary: s' vs' res ls rule: Lfilled.induct)
  case (L0 ves lholed es' es)
  obtain k where k_is:"(s, vs, (ves @ es) @ es') \<Down>k{(ls,r)} (s', vs', res)"
    using reduce_to_imp_reduce_to_n[OF L0(3)]
    by fastforce
  have "\<exists>s'' vs'' res'. ((s, vs, ves @ es) \<Down>{(ls,r)} (s'', vs'', res'))"
...
next
  ...
  obtain k' where k_is:"((s, vs, (ves @  [Label n es' lfilledk]) @ es'') \<Down>k'{(ls,r)} (s', vs', res))"
    using LN reduce_to_imp_reduce_to_n
    by simp blast
  obtain s'' vs'' res' vcs' ls where 1:"((s, vs, ($$*vcs') @ [Label n es' lfilledk]) \<Down>k'{(ls,r)} (s'', vs'', res'))"
  ...
  hence "\<exists>ls' vcs s'' vs'' res' lholed.
       ((s, vs, ($$* vcs) @ es) \<Down>{(ls', r)} (s'', vs'', res')) \<and>
...
lemma reduce_to_n_lfilled_context_equiv:
  assumes "Lfilled k lholed es les"
          ...
          "\<And>s'' vs'' res' ls vcs. ((s', vs', ($$* vcs)@es') \<Down>{(ls,r)} (s'', vs'', res')) \<Longrightarrow> ((s, vs, ($$* vcs)@es) \<Down>{(ls,r)} (s'', vs'', res'))"
...
proof (induction arbitrary: s' vs' res ls es' les' s_r vs_r rule: Lfilled.induct)
  ...
  consider
    (1) s'' vs'' rvs where
      "(s', vs', vs0 @ es') \<Down>{(ls, r)} (s'', vs'', RValue rvs)"
      "(s'', vs'', ($$* rvs) @ es0) \<Down>{(ls, r)} (s_r, vs_r, res)"
  | (2) "(s', vs', vs0 @ es') \<Down>{(ls, r)} (s_r, vs_r, res)"
...
next
  ...
  obtain lfilledk' where lfilledk'_def:"(les' = vs_ln @ [Label n esl_ln lfilledk'] @ es_ln)"
    ...
    by fastforce
  have 1: "(s', vs', (vs_ln @ [Label n esl_ln lfilledk']) @ es_ln) \<Down>{(ls, r)} (s_r, vs_r, res)"
  ...
  consider (a) s'' vs'' rvs where
              "(s', vs', ($$* vvs_ln) @ [Label n esl_ln lfilledk']) \<Down>{(ls, r)} (s'', vs'',  RValue rvs)"
              "(s'', vs'', ($$* rvs) @ es_ln) \<Down>{(ls, r)} (s_r, vs_r, res)"
         | (b) "(s', vs', ($$* vvs_ln) @ [Label n esl_ln lfilledk']) \<Down>{(ls, r)} (s_r, vs_r, res)"
               "(\<nexists>rvs. res = RValue rvs)"
  ...
  proof (cases)
    ...
    consider (aa) rvsaa where "(s', vs', lfilledk') \<Down>{(n # ls, r)} (s'', vs'', RValue rvsaa)"
      ...
      | (bb) rvsbb s''bb vs''bb res'bb vcs1bb vcs2bb where
                    ...
                    "(s''bb, vs''bb, ($$* vcs2bb) @ ($$* rvsbb) @ esl_ln) \<Down>{(ls, r)} (s'', vs'', res'bb)"
                    "(\<exists>rvs'. res'bb = RValue rvs' \<and>  RValue rvs = RValue (vcs1bb @ rvs'))"
      using reduce_to_label[OF a(1)]
      by fastforce
    hence "(s, vs, ($$* vvs_ln) @ [Label n esl_ln lfilledk]) \<Down>{(ls, r)} (s'', vs'', RValue rvs)"
    proof (cases)
      case aa
      show ?thesis
  ...
  next
    ...
    consider (aa) "(s', vs', lfilledk') \<Down>{(n # ls, r)} (s_r, vs_r, RTrap)"
      ...
      | (dd) rvsdd s''dd vs''dd vcs1dd vcs2dd where
            ...
            "(s''dd, vs''dd, ($$* vcs2dd) @ ($$* rvsdd) @ esl_ln) \<Down>{(ls, r)} (s_r, vs_r, res)"
      using reduce_to_label[OF b(1)] b(2)
      by fastforce
    hence "(s, vs, ($$* vvs_ln) @ [Label n esl_ln lfilledk]) \<Down>{(ls, r)} (s_r, vs_r, res)"
    proof (cases)
      case aa
      show ?thesis
        using LN(4)[OF lfilledk'_def(3) aa(1) LN(7)] aa(2)
    ...
    next
      case dd
      have 1:"(s''dd, vs''dd, ($$* vcs2dd @ rvsdd) @ esl_ln) \<Down>{(ls, r)} (s_r, vs_r, res)"
        using dd(3)
        by simp
      have 2:"(s, vs, ($$* vcs2dd) @  [Label n esl_ln lfilledk]) \<Down>{(ls, r)} (s_r, vs_r, res)"
...
lemma reduce_to_app_reduce_simple:
  assumes "\<lparr>es\<rparr> \<leadsto> \<lparr>es'\<rparr>"
          "((s,vs,($$*vcsf)@es') \<Down>{(ls,r)} (s',vs',res))"
  shows "((s,vs,($$*vcsf)@es) \<Down>{(ls,r)} (s',vs',res))"
  using assms
...
next
  ...
  have 1:"(s, vs, ves) \<Down>{(n#ls, r)} (s', vs', RValue vcs)"
    using vcs_def
...
next
  ...
  have 1:"(s, vls, es) \<Down>{([], Some n)} (s', vls, RValue vcs)"
    using vcs_def 0
...
next
  ...
  hence 1:"vs = vs'" "s = s'" "res = RValue (vcsf@vcs)" 
    ...
    by (metis map_append)+
  have "(s, vls, es) \<Down>{([], Some j)} (s, vls, RReturn vcs)"
...
next
  case (tee_local v j)
  obtain k where k_is:"((s, vs, ($$* vcsf) @  [v, v, $Set_local j]) \<Down>k{(ls, r)} (s', vs', res))"
    using tee_local(2) reduce_to_imp_reduce_to_n
    by blast
  obtain c where c_is:"v = $C c"
    using tee_local(1)
    by (metis e_type_const_unwrap)
  hence 0:"((s, vs, ($$* vcsf@[c]) @  [$C c, $Set_local j]) \<Down>k{(ls, r)} (s', vs', res))"
    using k_is c_is
    by auto
  have 1:"s = s'" "(f_locs vs)[j := c] = (f_locs vs')" "j < length (f_locs vs)" "res = RValue (vcsf@[c])" "f_inst vs = f_inst vs'"
    using reduce_to_n_set_local[OF 0]
    by auto
  hence
  "(s,\<lparr>f_locs = f_locs vs, f_inst = f_inst vs\<rparr>, [$C c, $C c,$Set_local j]) \<Down>{(ls, r)} (s', \<lparr>f_locs = f_locs vs', f_inst = f_inst vs'\<rparr>, RValue [c])"
    using 
      upd_conv_take_nth_drop[OF 1(3)] c_is id_take_nth_drop[OF 1(3)] c_is
      reduce_to.const_value[OF reduce_to.set_local[of "take j (f_locs vs)" j s "(f_locs vs)!j" "drop (Suc j) (f_locs vs)" "f_inst vs" c "(ls, r)"], of "[c]"]
    by simp
  hence "((s, vs, [v, v, $Set_local j]) \<Down>{(ls, r)} (s', vs', RValue [c]))"
    by (metis (full_types) "1"(1) "1"(5) c_is f.surjective old.unit.exhaust)
  thus ?case
...
next
  ...
  obtain k where res_b_def:"((s_l, f0, ($$* vcsf) @ [Local n fl es']) \<Down>k{(ls, r)} (s'', vs'', res))"
    using local(3) reduce_to_imp_reduce_to_n
    by blast
  then obtain lvs' lres where lres_def:"(s_l, fl, ($$*[])@es') \<Down>{([], Some n)} (s'', lvs', lres)"
                                       "f0 = vs''"
...
lemma reduce_trans_equiv_reduce_to_consts:
  shows "reduce_trans (s,vs,es) (s',vs',$$* vcs) = ((s,vs,es) \<Down>{([],None)} (s',vs',RValue vcs))"
proof -
  {
    assume local_assms:"(s, vs, es) \<Down>{([], None)} (s', vs', RValue vcs)"
    have "reduce_trans (s, vs, es) (s', vs', $$* vcs)"
